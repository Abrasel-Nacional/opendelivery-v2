...
- **Breaking**: `Service` payload shape changed in V2 (no service `id`; `type` and `status` as the base; schedules under `operatingHours`).
- **Breaking**: legacy V1 onboarding/status endpoints (`/v1/merchantOnboarding`, `/v1/merchantStatus`) are not part of the normative Merchant V2 contract.
...
- Migrate consumers to the new service shape (including status and schedule handling).
- Remove dependency on legacy onboarding/status endpoints and use discovery + normative Merchant operations.
- Use `GET .../snapshot` for bootstrap/reconciliation and CRUD for incremental updates.
...
### Orders V2 fields that must be in the migration plan

| Field | Type | Migration action | Impact |
|---|---|---|---|
| `Order.timing` | object | Move `orderTiming`, `schedule`, `preparationStartDateTime`, and `orderPriority` to the unified block | Structural payload breaking change |
| `Order.timing.schedule` | object | Enforce required rule when `orderTiming = SCHEDULED` | Mandatory conditional validation |
| `Order.context.salesChannel` | string | Map sales channel to `context.salesChannel` | Field location change |
| `Order.observations` | string | Separate order-level notes from item/context observations | New functional field |
| `Order.items[*].itemOfferId` | string | Send catalog item-offer reference when available | New optional field |
| `Order.items[*].observations` | string | Migrate from `specialInstructions` to `items[*].observations` | Semantic rename |
| `Order.items[*].options[*].defaultQuantity` | integer | Inform default included quantity before paid add-ons | New pricing-related field |
| `Order.items[*].options[*].options` | array[OrderItemOption] | Support multi-level option trees | New structural behavior |
| `Order.customer.birthDate` | string(date) | Map when available for CRM/Loyalty scenarios | New optional field |
| `Order.customer.gender` | string | Map when available for segmentation/communication | New optional field |
| `Order.customer` (when `fulfillment.orderType = DELIVERY`) | object | Make mandatory for delivery orders | Validation breaking change |
